...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods)
{
	if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_PRESS)
	{
		...
		if(ray.objs.size()>0)
		b->UnitSelected(ray.objs[0]->Position());
	}
}
#pragma endregion
...
void main()
{
	...
	//Creating models
	Model* empty = setupModel(4, glm::vec4(1.0f, 0.0f, 0.0f, 1.0f),1/3.0f);
	Model* modelX = setupModel(4, glm::vec4(0.0f, 0.0f, 1.0f, 1.0f),1 / 3.0f);
	Model* modelO = setupModel(20, glm::vec4(0.0f, 1.0f, 0.0f, 1.0f),1 / 4.0f);

	std::cout << "Enter the AI turn (1 or 2): ";
	int at;
	std::cin >> at;
	std::cout << "Player 1 is blue.\n";
	std::cout << "Player 2 is green.\n";
	std::cout << "Empty units are red.\n";

	b = new Board(empty,modelX,modelO,at);
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			bodies.push_back(b->tiles[i][j]);
		}
	}
